...
export function QuizListPage() {
  ...
  const { data: quizzes, isLoading } = useQuery({ queryKey: ['quizzes'], queryFn: quizzesApi.list })
  ...
  const statusMutation = useMutation({
    mutationFn: ({ id, status }: { id: string; status: QuizStatus }) =>
      quizzesApi.updateStatus(id, status),
    onSuccess: () => {
      invalidate()
      setActionError(null)
  ...
  const bulkDeleteMutation = useMutation({
    mutationFn: (ids: string[]) => Promise.all(ids.map((id) => quizzesApi.remove(id))),
    onSuccess: () => {
      invalidate()
      ...
      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-[12px] text-gray-400">
            Create, manage and organize quizzes for your workspace.
          </p>
        </div>
        <div className="flex shrink-0 items-center gap-2">
          ...
          <Button asChild>
            <Link to="/teacher/quizzes/new">New quiz</Link>
          </Button>
          <Popover>
            ...
            <Card className="rounded-md">
              <CardContent className="flex flex-col items-center gap-2 py-12 text-center">
                ...
                <p className="text-sm text-muted-foreground">
                  {all.length === 0 ? 'No quizzes yet.' : 'No quizzes match these filters.'}
                </p>
              </CardContent>
            </Card>
          ) : (
