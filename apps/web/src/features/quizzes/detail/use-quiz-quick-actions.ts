...
export function useQuizQuickActions(quiz: QuizDetail | undefined) {
  ...
  const duplicateMutation = useMutation({
    mutationFn: () => quizzesApi.duplicate(quiz!.id),
    onSuccess: (copy) => {
      queryClient.invalidateQueries({ queryKey: ['quizzes'] })
      navigate(`/teacher/quizzes/${copy.id}`)
    },
    onError: (err: unknown) =>
  ...
  const archiveMutation = useMutation({
    mutationFn: () => quizzesApi.updateStatus(quiz!.id, 'ARCHIVED'),
    onSuccess: () => {
      invalidate()
      setError(null)
    },
  ...
  const restoreMutation = useMutation({
    mutationFn: () => quizzesApi.updateStatus(quiz!.id, 'DRAFT'),
    onSuccess: () => {
      invalidate()
      setError(null)
    },
  ...
  const removeMutation = useMutation({
    mutationFn: () => quizzesApi.remove(quiz!.id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['quizzes'] })
  ...
  const previewQuiz = () => {
    window.open(`/teacher/quizzes/${quiz!.id}/print`, '_blank', 'noopener,noreferrer')
  }

  const exportPdf = () => {
    window.open(
      `/teacher/quizzes/${quiz!.id}/print?autoprint=1`,
      '_blank',
      'noopener,noreferrer',
    )
  }
