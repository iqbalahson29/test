...
import { AssignmentPanel } from '../../practice-assignments/assignment-panel'
...
export function QuizAssignTab({ quiz }: { quiz: QuizDetail }) {
  ...
  const invalidateQuiz = () => {
    ...
    queryClient.invalidateQueries({ queryKey: ['practice-quiz-activity', quiz.id] })
  }

  const statusMutation = useMutation({
    mutationFn: (status: QuizStatus) => practiceQuizzesApi.updateStatus(quiz.id, status),
    onSuccess: () => {
      invalidateQuiz()
      setStatusError(null)
    },
  ...
  const scheduleMutation = useMutation({
    mutationFn: async (iso: string) => {
      await practiceQuizzesApi.update(quiz.id, { availableFrom: iso })
      return practiceQuizzesApi.updateStatus(quiz.id, 'SCHEDULED')
    },
    onSuccess: () => {
      invalidateQuiz()
      setStatusError(null)
