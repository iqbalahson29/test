...
export function QuestionForm({
  ...
  /** True for a question-bank quiz — every draw must be auto-gradable, so
   * ESSAY/FILE_UPLOAD are hidden and difficulty becomes required. */
  bankMode?: boolean
  ...
  const submit = (e: FormEvent) => {
    ...
    if (bankMode && !difficulty) {
      setError('Difficulty is required for a question-bank quiz')
      return
    }
      ...
      <CardContent>
        <form onSubmit={submit} className="space-y-4">
          <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
              ...
              >
                ...
                <SelectContent>
                  {ALL_QUESTION_TYPES.filter(
                    (t) => !bankMode || (t !== QuestionType.ESSAY && t !== QuestionType.FILE_UPLOAD),
                  ).map((t) => (
            ...
            <div className="space-y-1.5">
              <Label htmlFor="question-difficulty">{bankMode ? 'Difficulty' : 'Difficulty (optional)'}</Label>
              <Select
              ...
              >
                ...
                <SelectContent>
                  {!bankMode && <SelectItem value={DIFFICULTY_UNSET}>Not set</SelectItem>}
