...
// FIXED: admin authors an exact question set every student sees identically.
// BANK: admin uploads a tagged-difficulty question pool and each attempt
// draws its own random SAT-sized subset (see BankModuleTargets/Ratio).
export type PracticeQuizMode = 'FIXED' | 'BANK'

export type BankModuleTargets = Record<QuizModule, number>

export interface BankDifficultyRatio {
  EASY: number
  MEDIUM: number
  HARD: number
}
...
export interface QuizDetail {
  ...
  status: QuizStatus
  mode: PracticeQuizMode
  bankModuleTargets: BankModuleTargets | null
  bankDifficultyRatio: BankDifficultyRatio | null
  createdAt: string
  updatedAt: string
...
export interface CreateQuizInput {
  ...
  /** Set once at creation, never changed afterward. */
  mode?: PracticeQuizMode
  bankModuleTargets?: BankModuleTargets
  bankDifficultyRatio?: BankDifficultyRatio
}
...
export interface CreateQuestionInput {
  ...
  difficulty?: QuestionDifficulty | null
  options?: OptionInput[]
  /** From practiceQuestionsApi.getAttachmentUploadUrl, after PUTting the file to S3. */
