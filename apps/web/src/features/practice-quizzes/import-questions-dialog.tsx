...
import type { CreateQuestionInput, PracticeQuizMode } from './types'
...
export function ImportQuestionsDialog({
  ...
  const { data: quizzes } = useQuery({
    queryKey: ['practice-quizzes'],
    queryFn: practiceQuizzesApi.list,
  ...
  const bankMode =
    (lockedQuizMode ?? quizzes?.find((q) => q.id === pickedQuizId)?.mode) === 'BANK'
  ...
  const importMutation = useMutation({
    mutationFn: async () => {
      ...
      return practiceQuestionsApi.import(quizId, toImport)
    },
    onSuccess: (res) => {
      setResult(res)
      setImageProgress(null)
  ...
  const onFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      ...
      const { questions, pendingImageFilenames, lines, errors } = parseQuestionsExcel(buffer, bankMode)
      setReady(questions)
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            {bankMode
              ? ' This is a question-bank quiz — essay and file-upload questions aren\'t allowed, and every row needs a Difficulty.'
              : ' Supports all question types.'}{' '}
            Each row's Module column assigns it to one of the 4 SAT modules.
          </DialogDescription>
        </DialogHeader>
        ...
        {!result && (
          <div className="flex items-center justify-between gap-3 rounded-lg border bg-muted/40 px-3 py-2.5">
            ...
            <Button
              ...
              size="sm"
              className="shrink-0"
              onClick={() => downloadQuestionsTemplate('questions-import-template', bankMode)}
            >
              <FileDown />
              Download template
            </Button>
