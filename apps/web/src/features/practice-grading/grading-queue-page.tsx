...
import { practiceGradingApi } from './api'
...
function GradingQueueRow({ item, quizId }: { item: GradingQueueItem; quizId: string }) {
  ...
  const gradeMutation = useMutation({
    mutationFn: () =>
      practiceGradingApi.grade(item.responseId, {
        awardedPoints: Number(points),
        feedback: feedback || undefined,
      }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['practice-grading-queue', quizId] })
    },
    onError: (err: unknown) =>
  ...
  const onDownload = async () => {
    ...
    setError(null)
    try {
      const { downloadUrl } = await practiceGradingApi.getDownloadUrl(item.responseId)
...
export function PracticeGradingQueuePage({ quizTitle }: { quizTitle: string }) {
  const { id } = useParams<{ id: string }>()
  const { data, isLoading } = useQuery({
    queryKey: ['practice-grading-queue', id],
    queryFn: () => practiceGradingApi.queue(id!),
    enabled: !!id,
  })
