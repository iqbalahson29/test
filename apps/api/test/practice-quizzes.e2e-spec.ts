...
describe('Practice quiz builder (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    betaAdminToken = (await createTenantAdmin(app, 'practicequizzes')).token;
  });
  ...
  it('builds a practice quiz containing all 9 question types, reorders, and publishes it', async () => {
    const createRes = await request(app.getHttpServer())
      .post('/practice-quizzes')
      .set('Authorization', `Bearer ${acmeAdminToken}`)
      .send({ title: 'Full Coverage Practice Quiz' })
      .expect(201);
    ...
    await request(app.getHttpServer())
      .patch(`/practice-quizzes/${quizId}/questions/reorder`)
      .set('Authorization', `Bearer ${acmeAdminToken}`)
    ...
    // Questions can still be added after publishing — PracticeQuestionsService
    // intentionally allows edits on a practice quiz in any status so teachers
    // can fix content on a live quiz; only quiz-level settings are locked to
    // DRAFT.
    await request(app.getHttpServer())
      .post(`/practice-quizzes/${quizId}/questions`)
      .set('Authorization', `Bearer ${acmeAdminToken}`)
  ...
  it('rejects invalid config and invalid option correctness per Zod schema / business rules', async () => {
    const createRes = await request(app.getHttpServer())
      .post('/practice-quizzes')
      .set('Authorization', `Bearer ${acmeAdminToken}`)
      .send({ title: 'Invalid Config Practice Quiz' })
      .expect(201);
    const quizId = createRes.body.id as string;
  ...
  it('blocks publishing a practice quiz with no questions', async () => {
    const createRes = await request(app.getHttpServer())
      .post('/practice-quizzes')
      .set('Authorization', `Bearer ${acmeAdminToken}`)
      .send({ title: 'Empty Practice Quiz' })
      .expect(201);
    const quizId = createRes.body.id as string;
  ...
  it('enforces tenant isolation on practice quizzes', async () => {
    const createRes = await request(app.getHttpServer())
      .post('/practice-quizzes')
      .set('Authorization', `Bearer ${acmeAdminToken}`)
      .send({ title: 'Acme Only Practice Quiz' })
      .expect(201);
    const quizId = createRes.body.id as string;
