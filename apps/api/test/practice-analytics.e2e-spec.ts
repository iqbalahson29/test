...
describe('Practice analytics (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    betaAdminToken = (await createTenantAdmin(app, 'practiceanalytics')).token;
  ...
  it('computes average/median/distribution/per-question stats correctly', async () => {
    // 10-point NUMERIC-only practice quiz, maxAttempts high enough for 2 attempts.
  ...
  it('enforces tenant isolation on both analytics endpoints', async () => {
    const createRes = await request(app.getHttpServer())
      .post('/practice-quizzes')
      .set('Authorization', `Bearer ${acmeAdminToken}`)
      .send({ title: 'Analytics Isolation Practice Quiz' })
      .expect(201);
