...
describe('Attempts (e2e)', () => {
  ...
  it('rejects starting an attempt for a quiz not assigned to the student', async () => {
    const createRes = await request(app.getHttpServer())
      .post('/quizzes')
      .set('Authorization', `Bearer ${acmeAdminToken}`)
  ...
  it('never leaks the answer key: options, matching pairs, accepted answers all stripped', async () => {
    const { quizId, mcqQuestionId } = await createAssignedQuiz(
      app,
      acmeAdminToken,
      studentMembershipId,
      'Sanitizer Test Quiz',
  ...
  it('blocks a new attempt once maxAttempts is exhausted', async () => {
    const { quizId } = await createAssignedQuiz(
      app,
      acmeAdminToken,
      studentMembershipId,
      'Max Attempts Quiz',
